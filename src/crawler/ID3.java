...
import java.util.TreeMap;
	...
	class myNode11{
		...
		List<defOneInstance11> listInstances;
		myNode11 Children[];
		myNode11 parent;
		//
		myNode11(myNode11 parent, List<defOneInstance11> lstInstances){
			testFts=-1;
			int cnt[] = { 0, 0 };
			Children = new myNode11[lstInstances.size()]; //re-check on the limit
			this.parent = parent;
			...
			for (defOneInstance11 currInstances: this.listInstances)
				//
		...
		//classify
		public int classify(defOneInstance11 t) {
	...
	class defOneInstance11{
		...
		// TODO Auto-generate11d constructor stub
		public defOneInstance11(int ID, String instanceLine) {
...
//id3
	public class ID3 {
			...
			public abstract static class baseImpure {
				...
				public double SPurity(int parentPos, int parentNeg) {
					// TODO Auto-generate11d method stub
					return 0;
				}
			}
			...
			public static TreeMap<Integer,List<defOneInstance11>> readFile2DS(String trainF, String testF,
																	 boolean FlagAtLast){
				File f=new File(trainF); String currLine="";
				int counter=0;
				List<defOneInstance11> listTrainInstances = new ArrayList<defOneInstance11>();
				List<defOneInstance11> listTestInstances = new ArrayList<defOneInstance11>();
				TreeMap<Integer,List<defOneInstance11>> mapOut = new TreeMap<Integer,List<defOneInstance11>>();
				...
				try{
					...
					while((currLine=inFile.readLine()) != null){
						//only samples get 
						counter++;
						defOneInstance11 currInstance=new defOneInstance11(counter, currLine);
	...
	//		public static myNode11 buildTree(List<defOneInstance11> listInstances, Impure  Impurefn) {
	//			myNode11 root = new myNode11(listInstances);
	//			
	...
	//		}
			...
			//
			public myNode11 generate11(List<defOneInstance11> instances, baseImpure p) {
				myNode11 root = new myNode11(null, instances);
				//l: no prune
				expand(root, p, 0);
				return root;
			}
			// more
			public void expand(myNode11 node, baseImpure p,
							   int depth) {
				...
				int mcount[][] = new int[defOneInstance11.FTSVALUERANGE][2];
				...
				for (int s = 0; s < node.AllFeaturesCnt; ++s) {
					...
					int count[][] = new int[defOneInstance11.FTSVALUERANGE][2];
					for (defOneInstance11  t : node.listInstances) {
					...
					//
					for (int i = 0; i < defOneInstance11.FTSVALUERANGE; i++) {
				...
				if (maxGain > 1e-10 &&  chiSquare(mcount)) {
					...
					//L:new array
					List<List<defOneInstance11>> ts = new  ArrayList<List<defOneInstance11>>();
					
					for (int i = 0; i < defOneInstance11.FTSVALUERANGE; ++i) {
						ts.add(new ArrayList<defOneInstance11>());
					}
					...
					for (int i = 0; i < defOneInstance11.FTSVALUERANGE; i++) {
						...
						if (ts.get(i).size() > 0) {
							node.Children[i] = new myNode11(node, ts.get(i));
			...
			//
//			public void learn11(List<defOneInstance11> instances, baseImpure p) {
//				this.root = generate11(instances, p );
//			}
			//
			public void learn11(List<defOneInstance11> instances) {
				this.root = generate11(instances, ID3.FnImpurity);
			}
			//classify
			public List<Integer> classify(List<defOneInstance11> testInstances) {
				List<Integer> predictions = new ArrayList<Integer>();
				for (defOneInstance11 t : testInstances) {
		...
		public static void main(String[] args) {
			...
			try{
				...
				TreeMap<Integer,List<defOneInstance11>> mapOut=readFile2DS(trainFile,testFile,FlagAtLast);
				//train Instances
				List<defOneInstance11> trainInstance=mapOut.get(1);
				List<defOneInstance11> testInstance=mapOut.get(2);
				
				ID3 myID= new ID3();
				//train
				myID.learn11(trainInstance);
				//test

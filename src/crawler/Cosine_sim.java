...
/**
 * This class is used for calculating similarity between two vectors,
 * finding the closest vector within a library of vectors and such, it 
 * can be used for any distance metric calculations
 * 
 * @author Barkin Aygun
 *
 */
public class Cosine_sim {
	 ...
	 * @param type Distance Metric for similarity (COSINE/L1/L2)
	 * @return the distance between the two vectors
	 */
	public static double calculateSimilarity(String[] vec1, String[] vec2, int type) {
		double similarity = 0;
		...
		for (int i = 0; i < vec1.length; i++) {
			switch (type) {
			case(COSINE):
				System.out.println("cosine switch");
				similarity +=new Double( vec1[i]) * new Double(vec2[i]);
				break;
			case(L2):
				similarity += Math.pow(new Double(vec1[i]) - new Double(vec2[i]), 2);
				break;
			case(L1):
				similarity += Math.abs(new Double(vec1[i]) - new Double(vec2[i]));
				break;
			}
		}
		if (type == COSINE)
			similarity = similarity / (vectorLength_string(vec1) * vectorLength_string(vec2));
		else if (type == L2)
	...
	private static double vectorLength_string(String[] vec) {
		double len = 0;
		for (int i = 0; i < vec.length; i++) {
			len += new Double( vec[i]) * new Double(vec[i]);
		}
		len = Math.sqrt(len);
		return len;			
	}
	 ...
	 * @return the distance between the two vectors
	 */
	public static double calculateSimilarity(String[] vec1, String[] vec2) {
		return calculateSimilarity(vec1, vec2, L2);
	}
	
	/**
	 * Finds the most similar vector to the given index vector in the vectors
	 * using the given distance metric, returns the indexes of vectors in descending
	 * order, closest first
	 * 
	 * @param vectors 2-D array of doubles representing all vectors
	 * @param index Index of the vector to find most similar to
	...
	/**
	 * Override for similarVectors using an ArrayList of vectors instead of 2-D array
	 *  
	 * @param vectors ArrayList of double arrays representing all vectors
	 * @param index Index of the vector to find most similar to
	 * @param type Distance metric to be used
	 * @return array of integers of indices of closest to furthest vector
	 */
	public static int[] similarVectors(ArrayList<String[]> vectors, int index, int type) {
		int[] indices = new int[vectors.size() - 1];
		int ind = 0;
		TreeMap<Integer, Double> unsortedSimilarities = new TreeMap<Integer, Double>();
		for (int i = 0; i < vectors.size(); i++) {
			if (i == index) continue;
			unsortedSimilarities.put(i, calculateSimilarity(vectors.get(index), 
															vectors.get(i), type));
		}
	...
	/**
	 * Override for similarVectors using the COSINE metric as default for ArrayList
	 * 
	 * @param vectors ArrayList of double arrays representing all vectors
	 * @param index Index of the vector to find most similar to
	 * @return array of integers of indices of closest to furthest vector
	 */
	public static int[] similarVectors(ArrayList<String[]> vectors, int index) {
		return similarVectors(vectors, index, COSINE);
	}
	 ...
	 * 
	 * @param vectors ArrayList of double arrays representing all vectors
	 * @param filename Location of the savefile
	 */
	public static void saveVectorsToFile(ArrayList<double[]> vectors, String filename) {
		FileWriter fw;
		double[] vector;
		try {
	 ...
	 * 
	 * @param filename Location of the vector file
	...
	/**
	 * The value comparer class that allows for sorting the vectors according to distance
	 * @author Barkin Aygun
	 *
	 */
